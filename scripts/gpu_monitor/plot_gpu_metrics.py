...
def main():
    ...
    # Crop periods of zero GPU utilization at beginning and end
    original_length = len(df)
    if args.crop:
        df = crop_zero_utilization(df, margin=args.margin)
        if len(df) < original_length:
            print(f"Data cropped from {original_length} to {len(df)} points to remove zero utilization periods")
    ...
    # Plot chart
    plot_gpu_metrics(df, args.output, args.show, args.title, comments)
